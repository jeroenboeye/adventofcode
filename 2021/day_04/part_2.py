...
def numbersheet_parser(text: str) -> Iterable[NumberSheet]:
    """Parse the input."""
    for sheet in text.split("\n\n")[1:]:
        rows = []
        columns: List[Set[int]] = [set() for _ in range(5)]
...
def solve(text: str) -> int:
    ...
    numbersheets = list(numbersheet_parser(text))
    sheets_remaining = len(numbersheets)
