...
def solve(text: str) -> int:
    """Solve the puzzle."""
    total = 0
    for knap_str in text.splitlines():
        comp_1 = set(knap_str[: len(knap_str) // 2])
        comp_2 = set(knap_str[len(knap_str) // 2 :])
        error_item = (comp_1 & comp_2).pop()
        total += ascii_letters.index(error_item) + 1
    return total
